...
namespace WorldMapStrategyKit {
	...
	public class WMSKTilesDownloader : EditorWindow {
		...
		int zoomLevelMin = WMSK.TILE_MIN_ZOOM_LEVEL;
		int zoomLevelMax = WMSK.TILE_MIN_ZOOM_LEVEL + 1;
		...
		void OnEnable () {
			Texture2D tex = Resources.Load<Texture2D> ("WMSK/WorldMapArea");
			if (tex != null) {
		...
		void PickCountryLatLon () {
			...
			if (s.Length >= 2) {
				...
				if (int.TryParse (s [1], out i)) {
					Rect rect = map.countries [i].mainRegion.rect2D;
					Vector2 latLonMin = Conversion.GetLatLonFromLocalPosition (rect.min);
					Vector2 latLonMax = Conversion.GetLatLonFromLocalPosition (rect.max);
					latMin = latLonMin.x;
					lonMin = latLonMin.y;
					latMax = latLonMax.x;
					lonMax = latLonMax.y;
