    ...
    public class WMSKInspector : Editor {
        ...
        void OnEnable() {
            ...
            earthStyleOptions = new string[] {
                "Solid Color", "Texture", "Natural", "Natural HighRes", "Natural HighRes 16K", "Alternate Style 1", "Alternate Style 2", "Alternate Style 3", "Scenic", "Scenic 8K", "Scenic 16K", "Viewport Scenic Plus", "Viewport Scenic Plus Alt 1", "Viewport Scenic Plus 16K"
            };
            earthStyleValues = new int[] {
                ...
                (int)EARTH_STYLE.NaturalScenic8K,
                (int)EARTH_STYLE.NaturalScenic16K,
        ...
        public override void OnInspectorGUI() {
            ...
            if (GUILayout.Button(new GUIContent("Collapse All", "Collapses all sections"), EditorStyles.miniButton)) {
                expandWindowSection = false;
                expandViewportSection = false;
                expandEarthSection = false;
                expandCitiesSection = false;
                expandCountriesSection = false;
                expandProvincesSection = false;
                expandTilesSection = false;
                expandInteractionSection = false;
                expandPathFindingSection = false;
                expandGrid = false;
                expandMiscellanea = false;
            }
            if (GUILayout.Button(new GUIContent("Documentation", "Browse online documentation."), EditorStyles.miniButton)) {
                Application.OpenURL("https://kronnect.com/guides-category/world-map-strategy-kit/");
            }
            if (GUILayout.Button("Support", EditorStyles.miniButton)) {
            ...
            if (expandWindowSection) {
                ...
                if (GUILayout.Button("Clear Constraints", GUILayout.Width(140))) {
                ...
                if (GUILayout.Button("Set SceneView Rect", GUILayout.Width(140))) {
            ...
            if (expandEarthSection) {
                ...
                if (_map.showEarth) {
                    ...
                    if (_map.earthStyle.numTextures() == 1) {
                        ...
                        _map.earthTextureScale = EditorGUILayout.Vector2Field("Texture Scale", _map.earthTextureScale);
                        _map.earthTextureOffset = EditorGUILayout.Vector2Field("Texture Offset", _map.earthTextureOffset);
                        EditorGUILayout.EndVertical();
                    }
                    if (_map.earthStyle.supportsBumpMap()) {
                        EditorGUILayout.BeginVertical(GUI.skin.box);
                        _map.earthBumpEnabled = EditorGUILayout.Toggle("Normal Map Enabled", _map.earthBumpEnabled);
                        if (_map.earthBumpEnabled) {
                            _map.earthBumpAmount = EditorGUILayout.Slider("   Amount", _map.earthBumpAmount, 0, 1f);
                            _map.earthBumpMapTexture = (Texture2D)EditorGUILayout.ObjectField("   Normal Map", _map.earthBumpMapTexture, typeof(Texture2D), false);
                        }
                        EditorGUILayout.EndVertical();
                    }

                    EditorGUILayout.BeginVertical(GUI.skin.box);
                    _map.showWater = EditorGUILayout.Toggle("Show Water", _map.showWater);
                    ...
                    EditorGUILayout.BeginHorizontal();
                    _map.scenicWaterMask = (Texture2D)EditorGUILayout.ObjectField(new GUIContent("Water Mask", waterMaskHelpText), _map.scenicWaterMask, typeof(Texture2D), false);
                    if (_map.earthStyle.isScenicPlus() && _map.scenicWaterMask != null) {
                        EditorGUILayout.BeginVertical();
                        if (GUILayout.Button("Help", GUILayout.Width(170))) {
                            EditorUtility.DisplayDialog("Water Mask", waterMaskHelpText, "Ok");
                        }
                        if (GUILayout.Button("Update Motion Vectors", GUILayout.Width(170))) {
                            UpdateMotionVectors();
                        }
                        EditorGUILayout.EndVertical();
                    }
                    ...
                    _map.heightMapTexture = (Texture2D)EditorGUILayout.ObjectField(new GUIContent("Height Map", "Heightmap is used to draw elevation in Render Viewport and for path-finding purposes when using elevation constraints."), _map.heightMapTexture, typeof(Texture2D), false);
                    EditorGUILayout.BeginVertical();
                    if (GUILayout.Button("Default 2K Heightmap", GUILayout.Width(170))) {
                        _map.heightMapTexture = Resources.Load<Texture2D>("WMSK/Textures/EarthHeightMap");
                    }
                    if (GUILayout.Button("Default 8K Heightmap", GUILayout.Width(170))) {
                        _map.heightMapTexture = Resources.Load<Texture2D>("WMSK/Textures/EarthHeightMap8K");
                    }
                    EditorGUILayout.EndVertical();
                    EditorGUILayout.EndHorizontal();
                }
            ...
            if (expandCountriesSection) {
                ...
                if (_map.showFrontiers) {
                    ...
                    if (_map.thickerFrontiers) {
                        _map.frontiersWidth = EditorGUILayout.Slider(new GUIContent("   Width", "Width multiplier for thick borders"), _map.frontiersWidth, 0.01f, 1f);
                        _map.frontiersMaxPixelWidth = EditorGUILayout.IntField(new GUIContent("   Max Pixel Width", "Maximum pixel width of the thick borders"), _map.frontiersMaxPixelWidth);
                        EditorGUILayout.HelpBox("'Thicker Borders' option currently requires geometry shader support which are not supported on Metal devices. Default frontier thickness will be used on these devices instead.", MessageType.Info);
                    }
                    if (!_map.renderViewportIsTerrain) {
                        _map.frontiersDynamicWidth = EditorGUILayout.Toggle(new GUIContent("   Dynamic Width", "Changes country frontiers shader LOD width when zooming in/out. Makes frontier rendering faster when zoomed out."), _map.frontiersDynamicWidth);
                    }
                    _map.combineCountrySurfaces = EditorGUILayout.Toggle(new GUIContent("   Combine Surfaces", "If enabled, surfaces of same country will be combined into a single mesh when calling ToggleCountrySurface to reduce draw calls. This will also disable individual region highlighting."), _map.combineCountrySurfaces);
                }
                ...
                if (_map.enableCountryHighlight) {
                    ...
                    _map.highlightCountryKeepTexture = EditorGUILayout.Toggle(new GUIContent("   Keep Texture", "When enabled, highlight will tint existing country texture."), _map.highlightCountryKeepTexture);
                    if (_map.combineCountrySurfaces) {
                        _map.highlightAllCountryRegions = true;
                        GUI.enabled = false;
                    }
                ...
                if (_map.showCountryNames) {
                    ...
                    EditorGUILayout.EndHorizontal();
                    _map.countryLabelsRenderingMode = (TEXT_RENDERING_MODE)EditorGUILayout.EnumPopup("   Rendering Mode", _map.countryLabelsRenderingMode);
                    if (_map.countryLabelsRenderingMode == TEXT_RENDERING_MODE.FloatingAboveViewport) {
                        if (_map.renderViewportIsEnabled) {
                            _map.countryLabelsElevation = EditorGUILayout.FloatField("   Labels Elevation", _map.countryLabelsElevation);
                            _map.countryLabelsAlwaysOnTop = EditorGUILayout.Toggle("   Always On Top", _map.countryLabelsAlwaysOnTop);
                            _map.countryLabelsUseMapCurvature = EditorGUILayout.Toggle(new GUIContent("   Use Map Curvature", "Curve labels to respect map curvature"), _map.countryLabelsUseMapCurvature);
                        } else {
                            EditorGUILayout.HelpBox("Floating labels are only supported in viewport mode.", MessageType.Info);
                        }
                    }
                    ...
                    _map.countryLabelsUseCentroid = EditorGUILayout.Toggle(new GUIContent("   Use Centroid", "Uses the centroid to position the label, instead of the geometric center."), _map.countryLabelsUseCentroid);
                    ...
                    if (_map.countryLabelsTextEngine == TEXT_ENGINE.TextMeshStandard) {
                        ...
                        _map.countryLabelsFontTMProMaterial = (Material)EditorGUILayout.ObjectField(new GUIContent("   Font Material (Optional)", "Font material preset to be used with TextMesh Pro"), _map.countryLabelsFontTMProMaterial, typeof(Material), false);
#endif
                    }
                    ...
                    if (_map.countryLabelsTextEngine == TEXT_ENGINE.TextMeshStandard) {
                        ...
                        if (_map.showLabelsShadow) {
                            ...
                            _map.countryLabelsShadowOffset = EditorGUILayout.Vector2Field("      Shadow Offset", _map.countryLabelsShadowOffset);
                        }
                    } else {
            ...
            if (expandProvincesSection) {
                ...
                if (_map.showProvinces) {
                    ...
                    _map.provincesDashBorders = EditorGUILayout.Toggle("   Dash Borders", _map.provincesDashBorders);
                    if (_map.provincesDashBorders) {
                        _map.provincesDashAmount = EditorGUILayout.Slider("      Dash Amount", _map.provincesDashAmount, 0, 1);
                    }
                }
                ...
                if (_map.enableProvinceHighlight) {
                    ...
                    _map.highlightProvinceKeepTexture = EditorGUILayout.Toggle(new GUIContent("   Keep Texture", "When enabled, highlight will tint existing province texture."), _map.highlightProvinceKeepTexture);
                ...
                if (_map.showProvinceNames) {
                    _map.countryLabelsUseCentroid = EditorGUILayout.Toggle(new GUIContent("   Use Centroid", "Uses the centroid to position the label, instead of the geometric center."), _map.countryLabelsUseCentroid); // currently we use the same setting for provinces and countries
                    ...
                    if (_map.provinceLabelsVisibility == PROVINCE_LABELS_VISIBILITY.Automatic) {
                        _map.showAllCountryProvinceNames = EditorGUILayout.Toggle(new GUIContent("   All Country Provinces", "Show labels of provinces of all countries"), _map.showAllCountryProvinceNames);
                    }
                    ...
                    if (_map.showProvinceLabelsShadow) {
                        ...
                        _map.provinceLabelsShadowOffset = EditorGUILayout.Vector2Field("      Shadow Offset", _map.provinceLabelsShadowOffset);
                    }

                    _map.provinceLabelsEnableAutomaticFade = EditorGUILayout.Toggle("   Auto Fade Labels", _map.provinceLabelsEnableAutomaticFade);
                    if (_map.provinceLabelsEnableAutomaticFade) {
                        _map.provinceLabelsAutoFadeMinHeight = EditorGUILayout.Slider("      Min Height", _map.provinceLabelsAutoFadeMinHeight, 0.01f, 0.25f);
                        _map.provinceLabelsAutoFadeMinHeightFallOff = EditorGUILayout.Slider("      Min Height Fall Off", _map.provinceLabelsAutoFadeMinHeightFallOff, 0.001f, _map.countryLabelsAutoFadeMinHeight);
                        _map.provinceLabelsAutoFadeMaxHeight = EditorGUILayout.Slider("      Max Height", _map.provinceLabelsAutoFadeMaxHeight, 0.1f, 1.0f);
                        _map.provinceLabelsAutoFadeMaxHeightFallOff = EditorGUILayout.Slider("      Max Height Fall Off", _map.provinceLabelsAutoFadeMaxHeightFallOff, 0.01f, 1f);
                    }
            ...
            if (expandTilesSection) {
                ...
                if (_map.showTiles) {
                    ...
                    if (_map.tileServer == TILE_SERVER.Custom) {
                        ...
                        if (_map.tileServer.IsAerisWeather()) {
                    ...
                    _map.tileRestrictToArea = EditorGUILayout.Toggle(new GUIContent("Restrict To Area", "Only display tiles for a certain area defined by lat/lon limits."), _map.tileRestrictToArea);
                    if (_map.tileRestrictToArea) {
                        Vector4 minMaxLatLon = _map.tileMinMaxLatLon;
                        EditorGUI.indentLevel++;
                        EditorGUI.BeginChangeCheck();
                        minMaxLatLon.x = EditorGUILayout.FloatField(new GUIContent("Latitude Min"), minMaxLatLon.x);
                        minMaxLatLon.z = EditorGUILayout.FloatField(new GUIContent("Latitude Max"), minMaxLatLon.z);
                        minMaxLatLon.y = EditorGUILayout.FloatField(new GUIContent("Longitude Min"), minMaxLatLon.y);
                        minMaxLatLon.w = EditorGUILayout.FloatField(new GUIContent("Longitude Max"), minMaxLatLon.w);
            ...
            if (expandGrid) {
                ...
                if (_map.showGrid) {
                    ...
                    _map.scenicWaterMask = (Texture2D)EditorGUILayout.ObjectField(new GUIContent("Water Mask", waterMaskHelpText), _map.scenicWaterMask, typeof(Texture2D), false);
                    _map.gridCutOutBorders = EditorGUILayout.Toggle(new GUIContent("Cut Out Borders", "Do not color part of cells on water"), _map.gridCutOutBorders);
                    ...
                    _map.gridMaxDistance = EditorGUILayout.FloatField(new GUIContent("Visible Distance Max", "10m = default max distance"), _map.gridMaxDistance);
                }
            }
            ...
            if (expandInteractionSection) {
                ...
                if (_map.allowUserZoom) {
                    ...
                    if (_map.enableCameraTilt) {
                        ...
                        if (_map.enableCameraOrbit) {
                            ...
                            _map.orbitYawAngle = EditorGUILayout.FloatField(new GUIContent("Yaw"), _map.orbitYawAngle);
                ...
                _map.allowInteractionWhileFlying = EditorGUILayout.Toggle(new GUIContent("Allow Interaction While Flying"), _map.allowInteractionWhileFlying);
                if (_map.allowInteractionWhileFlying) {
                    EditorGUI.indentLevel++;
                    _map.staticInteractionVelocityThreshold = EditorGUILayout.FloatField(new GUIContent("Velocity Threshold", "A maximum velocity of camera/map displacement to allow user interaction"), _map.staticInteractionVelocityThreshold);
                    EditorGUI.indentLevel--;
                }
            }
            ...
            if (!_map.renderViewportIsTerrain) {
                ...
                if (expandPathFindingSection) {
                    _map.enablePathFindingSystem = EditorGUILayout.Toggle("Enable Path-Finding System", _map.enablePathFindingSystem);
                    GUI.enabled = _map.enablePathFindingSystem;
                    ...
                    _map.pathFindingLandMap = (Texture2D)EditorGUILayout.ObjectField(new GUIContent("Land Map", "The landmap used to determine if a land-only unit can move."), _map.pathFindingLandMap, typeof(Texture2D), false);
                    _map.pathFindingWaterMap = (Texture2D)EditorGUILayout.ObjectField(new GUIContent("Water Map", "The watermap used to determine if water-only unit can move."), _map.pathFindingWaterMap, typeof(Texture2D), false);
            ...
            if (expandMiscellanea) {
                ...
                if (GUILayout.Button("Show", GUILayout.Width(60))) {
                    string path = GetGeodataFileSystemFolder();
        ...
        string GetGeodataFileSystemFolder() {
            string rootFolder;
            string path = "";
        ...
        void UpdateMotionVectors() {
            ...
            Texture2D terretrialTex = WMSK.instance.scenicWaterMask;
            if (terretrialTex == null) {
                EditorUtility.DisplayDialog("Unexpected Error", terretrialTex.name + " could not be found!\nThis should not happen - please make sure the file exists under Resources folder; if not, restore a copy from the original asset package and set the water mask value to 'WMSK/Textures/EarthScenicPlusMap8k'.", "Ok");
                return;
            }
            EditorCoroutines.Start(GenerateWaterMotionVectors(terretrialTex, motionVectorProgress, motionVectorFinish));
        }
        ...
        /// </summary>
        IEnumerator GenerateWaterMotionVectors(Texture2D tex, MotionVectorProgress progress, MotionVectorFinished finish) {
            ...
            if (!cancelled) {
                yield return EditorCoroutines.Start(BlurPass(colors, width, height, 1, 0, progress, "Pass 2/4"));
                if (progress != null)
                    progress(1, ""); // hide progress bar
            }
            if (!cancelled) {
                yield return EditorCoroutines.Start(BlurPass(colors, width, height, 0, 1, progress, "Pass 3/4"));
                if (progress != null)
                    progress(1, ""); // hide progress bar
            }
        ...
        static void motionVectorFinish(Texture2D terrestrialTex, bool cancelled) {
