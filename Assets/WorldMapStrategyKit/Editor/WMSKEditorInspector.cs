...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSKEditorInspector : Editor {
            ...
            if (selectedCountriesCount > 1) {
                ...
                if (_editor.countryIndex >= 0) {
                    ...
                    if (_editor.cityIndex >= 0) {
                        ...
                        _editor.GUICityLatitude = EditorGUILayout.FloatField("Latitude", _editor.GUICityLatitude);
                        _editor.GUICityLongitude = EditorGUILayout.FloatField("Longitude", _editor.GUICityLongitude);

                        _editor.GUICityPopulation = EditorGUILayout.IntField("Population", _editor.GUICityPopulation);
        ...
        void ShowReshapingMapTools () {
            ...
            if (GUILayout.Button(new GUIContent("Crop Countries", "Deletes any country whose center is outside of map bounds."))) {
                MapTransformCrop();
            }
            if (GUILayout.Button(new GUIContent("Apply & Save", "Apply scale and offset to coordinates of map entities."))) {
        ...
        static void RestoreBackupMenuOption (MenuCommand command) {
            ...
            if (!Directory.Exists(geoDataFolder)) {
                geoDataFolder = Directory.GetParent(backupFolder) + "/Resources/WMSK/Geodata";
                Directory.CreateDirectory(geoDataFolder);
                AssetDatabase.Refresh();
            }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/countries110.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
                CopyAsset(fullFileName, destFile);
            }
            // Countries110 binary
            fullFileName = backupFolder + "/countries110_bin.txt";
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/countries110_bin.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/countries10.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            // Countries10 binary
            fullFileName = backupFolder + "/countries10_bin.txt";
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/countries10_bin.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/" + WMSK.instance.countryAttributeFile + ".json";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/provinces10.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            // Provinces10 binary
            fullFileName = backupFolder + "/provinces10_bin.txt";
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/provinces10_bin.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/" + WMSK.instance.provinceAttributeFile + ".json";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/cities10.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            // Cities10 bin
            fullFileName = backupFolder + "/cities10_bin.txt";
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/cities10_bin.txt";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/" + WMSK.instance.cityAttributeFile + ".json";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
            ...
            if (File.Exists(fullFileName)) {
                string destFile = geoDataFolder + "/mountPoints.json";
                if (File.Exists(destFile)) {
                    AssetDatabase.DeleteAsset(destFile);
                    AssetDatabase.SaveAssets();
                }
        ...
        static void CopyAsset (string src, string dst) {
            int i = src.IndexOf("Assets");
            if (i >= 0) src = src.Substring(i);
            int j = dst.IndexOf("Assets");
            if (j >= 0) dst = dst.Substring(j);
        ...
        void ExecuteMoveTool (Vector2 mousePosition) {
            ...
            for (int i = 0; i < points.Length; i++) {
                ...
                var fmh_2081_61_638746353386001716 = mapTransform.rotation; newPoint = Handles.FreeMoveHandle(oldPoint, handleSize, pointSnap,
        ...
        void ExecuteCityMoveTool () {
            ...
            var fmh_2268_65_638746353386019701 = mapTransform.rotation; Vector3 newPoint = Handles.FreeMoveHandle(oldPoint, handleSize, pointSnap,
                                                   (handleControlID, position, rotation, size, eventType) => {
        ...
        void ExecuteMountPointMoveTool () {
            ...
            var fmh_2355_65_638746353386024678 = mapTransform.rotation; Vector3 newPoint = Handles.FreeMoveHandle(oldPoint, handleSize, pointSnap,
                                                                   (handleControlID, position, rotation, size, eventType) => {
                                                                       Handles.RectangleHandleCap(handleControlID, position, rotation, size, eventType);
                                                                   });
            if (newPoint != oldPoint) {
        ...
        // Add this method near the other editor methods
        void UpdateCityChanges () {
            if (_editor.cityIndex < 0 || _editor.cityIndex >= _map.cities.Length) return;

            City city = _map.cities[_editor.cityIndex];
            bool hasChanges = false;

            // Check if name changed
            _editor.GUICityName = _editor.GUICityNewName.Trim();
            if (city.name != _editor.GUICityNewName) {
                hasChanges = true;
            }

            // Check if class changed
            if (city.cityClass != _editor.GUICityClass) {
                hasChanges = true;
            }

            // Check if location changed
            Vector2 newLocation = Conversion.GetLocalPositionFromLatLon(_editor.GUICityLatitude, _editor.GUICityLongitude);
            if (city.unity2DLocation != newLocation) {
                hasChanges = true;
            }

            // Check if population changed
            if (city.population != _editor.GUICityPopulation) {
                hasChanges = true;
            }

            // Only perform updates if there are actual changes
            if (hasChanges) {
                UndoPushCityStartOperation("Undo City Changes");

                city.name = _editor.GUICityNewName;
                city.cityClass = _editor.GUICityClass;
                city.unity2DLocation = newLocation;
                city.population = _editor.GUICityPopulation;

                _editor.ReloadCityNames();
                _editor.map.DrawCities();
                _editor.cityChanges = true;
