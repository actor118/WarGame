...
namespace WorldMapStrategyKit {
	public class DemoLoadSave : MonoBehaviour {
        ...
        /// </summary>
        byte[] countryGeoData, provinceGeoData, cityGeoData;
        string countryAttributes, provinceAttributes, cityAttributes, mountPointGeoData, mountPointAttributes;
		int state;
		...
		void Start () {
			...
			#if UNITY_EDITOR
			EditorUtility.DisplayDialog ("Load/Save Demo", "In this demo scene, a map change is simulated (North America is collapsed into one single country), then saved and loaded.\n\nTo start, press the top/left button on the screen.", "Ok");
			#endif
		}
		...
		void SaveAllData () {
			...
			countryGeoData = map.GetCountriesGeodataBinary();
			...
			provinceGeoData = map.GetProvincesGeodataBinary();
			...
			cityGeoData = map.GetCitiesGeodataBinary();
		...
		void LoadAllData () {
			...
			map.SetCountriesGeodataBinary (countryGeoData);
			map.SetCountriesAttributes (countryAttributes);

			// Same for provinces. This wouldn't be neccesary if you are not using provinces in your app.
			map.SetProvincesGeodataBinary(provinceGeoData);
			map.SetProvincesAttributes (provinceAttributes);

			// Same for cities. This wouldn't be neccesary if you are not using cities in your app.
			map.SetCitiesGeodataBinary(cityGeoData);
