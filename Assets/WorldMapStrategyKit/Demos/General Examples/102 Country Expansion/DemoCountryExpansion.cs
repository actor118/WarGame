...
namespace WorldMapStrategyKit {
    public class DemoCountryExpansion : MonoBehaviour {
        ...
        IEnumerator StartExpansion() {
            ...
            while (regionFound) {
                regionFound = false;
                Country country = map.countries[principalCountryIndex];
                for (int r = 0; r < country.regions.Count; r++) {
                    Region region = country.regions[r];
                    foreach (Region otherRegion in region.neighbours) {
                        if (!autoExpand)
                            yield break;
        ...
        IEnumerator TransferCountry() {
            ...
            LineMarkerAnimator lma = map.AddLine(new Vector2[]
                {
                    map.countries[sourceCountryIndex].centroid,
                    map.countries[targetCountryIndex].centroid
