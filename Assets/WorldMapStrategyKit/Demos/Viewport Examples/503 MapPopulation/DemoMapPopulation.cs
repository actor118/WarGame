...
namespace WorldMapStrategyKit {
	...
	public class DemoMapPopulation : MonoBehaviour {
		...
		GameObjectAnimator tank, ship, airplane;
		...
		void OnGUI () {
			...
			if (GUI.Button(new Rect(10, 250, 150, 30), "  Create Airplane", buttonStyle)) {
				ShowAirplane();
			}

			if (airplane != null) {
				if (GUI.Button(new Rect(10, 290, 150, 30), "  Start Flight", buttonStyle)) {
					StartFlight();
				}
				if (GUI.Button(new Rect(10, 330, 150, 30), "  Move Airplane", buttonStyle)) {
					MoveAirplane();
				}
			}
		...
		void AddRandomSpriteAtPosition (Vector2 position) {
			...
			GameObject star = Instantiate(spritePrefab);
		...
		void AddTowerAtPosition (float x, float y) {
			...
			GameObjectAnimator anim = tower.WMSK_MoveTo(x, y, true);
		}
		...
		void MoveShipToDestination (Vector2 destination, float duration) {
		...
		void ShowAirplane () {
			...
			Vector2 position = map.GetCity("New York", "United States of America").unity2DLocation;

			// Create airplane
			GameObject airplaneGO = Instantiate(airplanePrefab);
			airplaneGO.transform.localScale = Misc.Vector3one * 0.5f;
			airplane = airplaneGO.WMSK_MoveTo(position);
			airplane.type = (int)UNIT_TYPE.AIRPLANE;              // this is completely optional, just used in the demo scene to differentiate this unit from other tanks and ships
			...
			airplane.heightMode = HEIGHT_OFFSET_MODE.ABSOLUTE_CLAMPED;
			...
			airplane.arcMultiplier = 2f;     // this is the arc for the plane trajectory
		...
		void StartFlight () {
			...
			airplane.TeleportTo(position);
			airplane.follow = true;

			Vector2 destination = map.GetCity("Paris", "France").unity2DLocation;
			airplane.MoveTo(destination, duration: 15f);
		...
		void MoveAirplane () {
			...
			Vector2 destination = map.GetCityRandom().unity2DLocation;
			airplane.MoveTo(destination, 35f, DURATION_TYPE.MapLap);
			airplane.follow = true;

			LineMarkerAnimator line = map.AddLine(airplane.currentMap2DLocation, destination, Color.yellow, arcMultiplier: 0, lineWidth: 0.1f); 
			line.dashInterval = 0.001f;
			line.dashAnimationDuration = 1f;
		...
		/// </summary>
		void MassCreate (int numberOfUnits) {
			...
			for (int k = 0; k < numberOfUnits; k++) {
				...
				GameObjectAnimator newShip = DropShipOnPosition(waterPosition, false);
