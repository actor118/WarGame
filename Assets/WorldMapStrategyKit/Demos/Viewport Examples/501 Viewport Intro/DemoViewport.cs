...
namespace WorldMapStrategyKit {
    public class DemoViewport : MonoBehaviour {
        ...
        void OnGUI() {
            ...
            // Add marker sample
            if (GUI.Button(new Rect(GUIResizer.authoredScreenWidth - 190, 220, 180, 30), "  Add Marker (Sprite)", buttonStyle)) {
                AddMarkerOnRandomCity();
            }

            // Add marker sample (Text)
            if (GUI.Button(new Rect(GUIResizer.authoredScreenWidth - 190, 260, 180, 30), "  Add Marker (Text)", buttonStyle)) {
                AddMarkerTextOnRandomPlace();
            }

            if (GUI.Button(new Rect(GUIResizer.authoredScreenWidth - 190, 300, 180, 30), "  Add Trajectories", buttonStyle)) {
                AddTrajectories();
            }
        ...
        void AddMarkerOnRandomCity() {
            ...
            map.OnMarkerMouseDown += (marker, buttonIndex) => Debug.Log("Mouse down on sprite with button " + buttonIndex + "!");
            map.OnMarkerMouseUp += (marker, buttonIndex) => Debug.Log("Mouse up on sprite with button " + buttonIndex + "!");
            map.OnMarkerMouseEnter += (marker) => Debug.Log("Pointer is on sprite!");
            map.OnMarkerMouseExit += (marker) => Debug.Log("Pointer exits sprite!");
        ...
        void AddTrajectories() {
            ...
            for (int line = 0; line < 5; line++) {
                ...
                Color color = new Color(Random.Range(0f, 0.75f), Random.Range(0f, 0.75f), Random.Range(0f, 0.75f));
                float elevation = Random.Range(1f, 10f);
                ...
                if (Random.value > 0.5f) {  // make it a dash line
        ...
        void LocateMountPoint() {
            ...
            if (mountPointsCount > 0) {
                ...
                int mp = Random.Range(0, mountPointsCount - 1);
