...
namespace WorldMapStrategyKit {
    public class DemoStandAlone : MonoBehaviour {
        ...
        void OnGUI() {
            ...
            if (GUI.Button(new Rect(10, 520, 180, 30), "  Country Interior Shadow", buttonStyle)) {
                DrawCountryInteriorShadow();
            }
            ...
            if (GUI.Button(new Rect(GUIResizer.authoredScreenWidth - 190, 500, 180, 30), "  Draw Combined Circles", buttonStyle)) {
                DrawCombinedCircles();
            }

            if (GUI.Button(new Rect(GUIResizer.authoredScreenWidth - 190, 540, 180, 30), "  Draw Blended Circles", buttonStyle)) {
                DrawBlendedCircles();
            }
        ...
        void Update() {
            if (map.input.GetKeyDown(KeyCode.G)) {  // press G to show/hide GUI buttons
                avoidGUI = !avoidGUI;
            }
            if (map.input.GetKeyDown(KeyCode.H)) {
                for (int k = 0; k < map.countries.Length; k++) {
                    map.ToggleCountryOutline(k, true, gradient);
                }
            }
        }

        void DrawCombinedCircles() {
            Vector2 v = map.GetCountry("Spain").center;
            Vector2 w = v + new Vector2(0.015f, 0);
            Color red = new Color(1, 0, 0, 0.5f);
            Color yellow = new Color(1, 1, 0, 0.5f);
            map.AddCircle(v, 1000, 0, 0.9f, red, overdraw: false);
            map.AddCircle(w, 1000, 0, 0.9f, red, overdraw: false);
            map.AddCircle(v, 1000, 0.9f, 1f, yellow, overdraw: false, renderOrder: 2);
            map.AddCircle(w, 1000, 0.9f, 1f, yellow, overdraw: false, renderOrder: 2);
        }

        void DrawBlendedCircles() {
            Vector2 v = map.GetCountry("Germany").center;
            Vector2 w = v + new Vector2(0.015f, 0);
            Color red = new Color(1, 0, 0, 0.5f);
            Color yellow = new Color(1, 1, 0, 0.5f);
            map.AddCircle(v, 1000, 0f, 1f, red);
            map.AddCircle(w, 1000, 0f, 1f, yellow);
        }
        ...
        void TextureSample() {
            ...
            map.ToggleCountryMainRegionSurface(countryIndex, true, Color.white, texture);
        ...
        void AddMarkerSpriteOnRandomCity() {
            ...
            //		planeLocation = Conversion.GetLocalPositionFromLatLon(40.71f, -74.00f);
            ...
            map.AddMarker2DSprite(star, planeLocation, 0.02f, enableEvents: true, autoScale: true);
        ...
        /// In this example a building prefab is added to a random city (see comments for other options).
        /// </summary>
        void AddMarker3DObjectOnRandomCity() {
        ...
        /// </summary>
        void ShowCountryBorders() {
            int india = map.GetCountryIndex("India");
            ...
            map.Redraw(true);
            float zoomLevel = map.GetCountryRegionZoomExtents(india);
        ...
        /// </summary>
        void TransparentTextureFill() {
            int china = map.GetCountryIndex("China");
            map.ToggleCountryMainRegionSurface(china, true, gradient);
            float zoomLevel = map.GetCountryRegionZoomExtents(china);
            map.FlyToCountry(china, 0, zoomLevel + 0.02f);
        }


        void DrawCountryInteriorShadow() {
            int ci = map.GetCountryIndex("Spain");
            Region region = map.countries[ci].mainRegion;
            Texture outlineTexture = Resources.Load<Texture>("WMSK/Textures/Outline/GradientInterior");
            map.DrawRegionOutline("Border", region, borderTexture: outlineTexture, texturedBorderWidth: 0.5f, tintColor: new Color(0, 0, 0, 0.65f));
            map.FlyToCountry(ci, 2f, 0.1f);
        }
