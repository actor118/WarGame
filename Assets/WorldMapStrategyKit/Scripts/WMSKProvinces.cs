...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        /// Returns the province object by its name.
        /// </summary>
        public Province GetProvince(string provinceName, int countryIndex) {
        ...
        public List<Province> ProvinceNeighboursOfRegion(Region provinceRegion) {
            ...
            foreach (Region neighbour in provinceRegion.neighbours) {
        ...
        /// <summary>
        /// Colorize main region of a province by index in the countries collection.
        /// </summary>
        public GameObject ToggleProvinceMainRegionSurface(int provinceIndex, bool visible, SurfaceOptions options) {
            return internal_ToggleProvinceRegionSurface(provinceIndex, provinces[provinceIndex].mainRegionIndex, visible, options);
        }


        /// <summary>
        /// Colorize region of a province by index in the countries collection.
        /// </summary>
        public GameObject ToggleProvinceRegionSurface(int provinceIndex, int regionIndex, bool visible, SurfaceOptions options) {
            return internal_ToggleProvinceRegionSurface(provinceIndex, regionIndex, visible, options);
        }
        ...
        public GameObject ToggleProvinceRegionSurface(int provinceIndex, int regionIndex, bool visible, Color color, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
            surfOptions.color = color;
            surfOptions.texture = texture;
            surfOptions.textureScale = textureScale;
            surfOptions.textureOffset = textureOffset;
            surfOptions.textureRotation = textureRotation;
            surfOptions.useSDFGradient = false;
            return internal_ToggleProvinceRegionSurface(provinceIndex, regionIndex, visible, surfOptions);
        }


        GameObject internal_ToggleProvinceRegionSurface(int provinceIndex, int regionIndex, bool visible, SurfaceOptions options) {
            if (!ValidProvinceRegionIndex(provinceIndex, regionIndex)) return null;
            ...
            if (surf != null) {
                ...
                if (options.texture != null && (options.textureScale != region.customTextureScale || options.textureOffset != region.customTextureOffset || options.textureRotation != region.customTextureRotation || surfMaterial.name.Equals(coloredMat.name))) {
                    DestroyImmediate(surf);
                    surf = null;
                }
            }
            ...
            Material goodMaterial = GetColoredTexturedMaterial(options.color, options.texture, options.useSDFGradient, true, 1);
            ...
            if (surf != null) {
                ...
                surfMaterial = surf.GetComponent<Renderer>().sharedMaterial;
                if ((options.texture == null && !surfMaterial.name.Equals(coloredMat.name)) || (options.texture != null && !surfMaterial.name.Equals(texturizedMat.name))
                    || (surfMaterial.color != options.color && !isHighlighted) || (options.texture != null && region.customMaterial.mainTexture != options.texture)) {
                    ApplyMaterialToSurface(surf, goodMaterial);
                }
            } else {
                surf = GenerateProvinceRegionSurface(provinceIndex, regionIndex, goodMaterial, options.textureScale, options.textureOffset, options.textureRotation);
                region.customMaterial = goodMaterial;
                region.customTextureOffset = options.textureOffset;
                region.customTextureRotation = options.textureRotation;
                region.customTextureScale = options.textureScale;
            }
            ...
            // Add SDF data if needed
            Material mat = region.customMaterial;
            if (options.useSDFGradient) {
                if (region.sdfTexture == null) {
                    region.sdfTexture = SDFTextureGenerator.Generate(region, options.sdfQuality);
                }
                mat.SetTexture(ShaderParams.SDFTexture, region.sdfTexture);
                mat.SetFloat(ShaderParams.SDFBorderFallOff, options.sdfBorderFallOff);
                mat.SetColor(ShaderParams.SDFBorderColor, options.sdfBorderColor);
                mat.EnableKeyword(ShaderParams.SKW_SDF_TEXTURE);
            } else {
                mat.DisableKeyword(ShaderParams.SKW_SDF_TEXTURE);
            }
        ...
        public List<Vector2> GetProvinceBorderPoints(int provinceIndex1, int provinceIndex2, int extraWidth = 0) {
            ...
            // if first point is nearest to the latest point, add it to the end
            int count = samePoints.Count;
            if (count > 2) {
                Vector2 p0 = samePoints[0];
                Vector2 p1 = samePoints[1];
                Vector2 p2 = samePoints[count - 1];
                if (Vector2.Distance(p0, p2) < Vector2.Distance(p0, p1)) {
                    samePoints.RemoveAt(0);
                    samePoints.Add(p0);
                }
            }
        ...
        public void SetProvincesGeodataBinary(byte[] data) {
            ...
            for (int k = 0; k < provinceCount; k++) {
                ...
                for (int r = 0; r < regionsCount; r++) {
                    ...
                    provinceRegion.UpdatePointsAndRect(newPoints);
                ...
                province.uniqueId = br.ReadInt32();
