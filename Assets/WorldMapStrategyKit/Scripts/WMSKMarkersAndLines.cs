...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        public TextMeshPro AddMarker2DTextPro(string text, Vector3 planeLocation, Color color, float scale = 0.14f, TMP_FontAsset font = null, FontStyles fontStyle = FontStyles.Normal) {
            CheckMarkersLayer();
        ...
        public LineMarkerAnimator AddLine(Vector2 start, Vector2 end, Color color, float arcMultiplier, float lineWidth) {
            Vector2[] path = new Vector2[] { start, end };
            LineMarkerAnimator lma = AddLine(path, markerLineMat, arcMultiplier, lineWidth);
            lma.color = color;
            return lma;
        }
        ...
        /// <param name="arcElevation">arc elevation (-0.5 .. 0.5)</param>
        public LineMarkerAnimator AddLine(Vector2[] points, Color color, float arcMultiplier, float lineWidth) {
            LineMarkerAnimator lma = AddLine(points, markerLineMat, arcMultiplier, lineWidth);
            lma.color = color;
            return lma;
        }
        ...
        /// <param name="Color">line color</param>
        /// <param name="arcElevation">arc elevation (-0.5 .. 0.5)</param>
        public LineMarkerAnimator AddLine(List<Vector2> points, Color color, float arcMultiplier, float lineWidth) {
            return AddLine(points.ToArray(), color, arcMultiplier, lineWidth);
        }
...
int lineCounter;
        ...
        public LineMarkerAnimator AddLine(Vector2[] points, Material lineMaterial, float arcElevation, float lineWidth) {
            ...
            if (line == null) {
                line = new GameObject("MarkerLine" + lineCounter++, typeof(LineMarkerAnimator));
                LinesPool.Push(line);
            }
        ...
        /// <param name="renderOrder">Order for rendering this circle</param>
        /// <param name="enableEvents">Circle will emit click events through the marker events: OnMarkerClick, etc.</param>
        public GameObject AddCircle(Vector2 position, float kmRadius, float ringWidthStart, float ringWidthEnd, Color color, bool overdraw = true, int renderOrder = 0, bool enableEvents = false) {
            CheckMarkersLayer();
        ...
        /// <param name="renderOrder">Order for rendering this circle</param>
        /// <param name="enableEvents">Circle will emit click events through the marker events: OnMarkerClick, etc.</param>
        public GameObject AddCircleOnSphere(Vector2 position, float kmRadius, float ringWidthStart, float ringWidthEnd, Color color, bool overdraw = true, int renderOrder = 0, bool enableEvents = false) {
            CheckMarkersLayer();
            Material mat = GetColoredMarkerMaterial(color);
            if (!overdraw || renderOrder != 0) {
                mat = Instantiate(mat);
