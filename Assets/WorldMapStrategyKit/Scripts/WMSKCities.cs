...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        public void SetCitiesGeodata(string s) {
            ...
            int cityCount = s.Count('|') + 1;
            _cities = new City[cityCount];
            int cityIndex = 0;
            int unknownCountryIndex = -1;
            int k = 0;
            StringSegment[] cityInfo = new StringSegment[8];
            foreach (var cityData in s.GetSplit('|')) {
                s.GetSplitSegments(cityData, '$', cityInfo);
                string country = s.GetSplitEntry(cityInfo[2]);
                int countryIndex = GetCountryIndex(country);
                if (countryIndex < 0) {
                ...
                if (countryIndex >= 0) {
                    string name = s.GetSplitEntry(cityInfo[0]);
                    string province = s.GetSplitEntry(cityInfo[1]);
                    int population = int.Parse(s.GetSplitEntry(cityInfo[3]), Misc.InvariantCulture);
                    float x = float.Parse(s.GetSplitEntry(cityInfo[4]), Misc.InvariantCulture);
                    float y = float.Parse(s.GetSplitEntry(cityInfo[5]), Misc.InvariantCulture);
                    CITY_CLASS cityClass = (CITY_CLASS)int.Parse(s.GetSplitEntry(cityInfo[6]), Misc.InvariantCulture);
                    int uniqueId;
                    if (cityInfo.Length >= 8) {
                        uniqueId = int.Parse(s.GetSplitEntry(cityInfo[7]), Misc.InvariantCulture);
                    } else {
        ...
        public byte[] GetCitiesGeodataBinary() {
            ...
            for (int k = 0; k < citiesCount; k++) {
                ...
                bw.Write(city.unity2DLocation.x);
                bw.Write(city.unity2DLocation.y);
                bw.Write((byte)city.cityClass);
                bw.Write(city.uniqueId);
