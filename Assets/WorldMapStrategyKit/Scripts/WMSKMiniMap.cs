...
namespace WorldMapStrategyKit {
    ...
    public class WMSKMiniMap : MonoBehaviour {
		...
		public static WMSKMiniMap instance {
            get {
                if (_instance == null) {
                    _instance = Misc.FindObjectOfType<WMSKMiniMap>();
                }
                return _instance;
            }
        }
        ...
        readonly Vector3[] wc = new Vector3[4];
        ...
        /// <summary>
        /// Refresh mini-map parent
        /// </summary>
        public static void Reposition() {
            WMSKMiniMap mm = instance;
            if (mm != null) {
                RepositionAt(mm.normalizedScreenRect);
            }
        }
        ...
        public static void RepositionAt(Vector4 normalizedScreenRect) {
            WMSKMiniMap mm = instance;
            if (mm == null) return;
            WMSK primaryMap = WMSK.instance;
            if (primaryMap != null) {
                Camera parentCamera = primaryMap.enableCameraTilt ? primaryMap.viewCamera : primaryMap.cameraMain;
                if (parentCamera != null) {
                    Transform t = parentCamera.transform;
                    if (t != mm.transform.parent) {
                        mm._normalizedScreenRect = Vector4.zero;
                        mm.transform.SetParent(t, false);
                    }
                }
            }
            mm.normalizedScreenRect = normalizedScreenRect;
        }
        ...
        void RepositionMiniMap() {
            Camera cameraMain = WMSK.instance.viewCamera;
