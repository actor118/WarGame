...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        [SerializeField]
        bool _countryLabelsAlwaysOnTop;

        public bool countryLabelsAlwaysOnTop {
            get {
                return _countryLabelsAlwaysOnTop;
            }
            set {
                if (_countryLabelsAlwaysOnTop != value) {
                    _countryLabelsAlwaysOnTop = value;
                    isDirty = true;
                    DrawMapLabels();
                }
            }
        ...
        [SerializeField]
        bool _countryLabelsUseMapCurvature;

        public bool countryLabelsUseMapCurvature {
            get {
                return _countryLabelsUseMapCurvature;
            }
            set {
                if (_countryLabelsUseMapCurvature != value) {
                    _countryLabelsUseMapCurvature = value;
                    isDirty = true;
                    DrawMapLabels();
                }
            }
        ...
        public List<Country> CountryNeighboursOfRegion(Region countryRegion) {
            ...
            foreach (Region neighbour in countryRegion.neighbours) {
        ...
        /// </summary>
        public GameObject ToggleCountryMainRegionSurface(int countryIndex, bool visible, SurfaceOptions options) {
            if (!ValidCountryIndex(countryIndex)) return null;
            return internal_ToggleCountryRegionSurface(countryIndex, _countries[countryIndex].mainRegionIndex, visible, options);
        }


        /// <summary>
        /// Colorize region of a country by index in the countries collection.
        /// </summary>
        public GameObject ToggleCountryRegionSurface(int countryIndex, int regionIndex, bool visible, SurfaceOptions options) {
            return internal_ToggleCountryRegionSurface(countryIndex, regionIndex, visible, options);
        }

        /// <summary>
        /// Colorize main region of a country by index in the countries collection.
        /// </summary>
        public GameObject ToggleCountryMainRegionSurface(int countryIndex, bool visible, Color color = default) {
        ...
        public GameObject ToggleCountryMainRegionSurface(int countryIndex, bool visible, Color color, Texture2D texture) {
            if (!ValidCountryIndex(countryIndex)) return null;
            return ToggleCountryRegionSurface(countryIndex, _countries[countryIndex].mainRegionIndex, visible, color, texture, Misc.Vector2one, Misc.Vector2zero, 0);
        }
        ...
        /// Colorize region of a country by index in the countries collection.
        /// </summary>
        public GameObject ToggleCountryRegionSurface(int countryIndex, int regionIndex, bool visible, Color color = default) {
        ...
        public void BlinkCountry(int countryIndex, int regionIndex, Color color1, Color color2, float duration, float blinkingSpeed, bool smoothBlink = true) {
			...
			if (surf.TryGetComponent(out SurfaceBlinker sb)) DestroyImmediate(sb);
        ...
        public List<Vector2> GetCountryFrontierPoints(int countryIndex1, int countryIndex2, int extraWidth = 0) {
            ...
            for (int cr = 0; cr < country1RegionsCount; cr++) {
                ...
                foreach(Region otherRegion in region1.neighbours) {
        ...
        /// <param name="deleteSourceCountryIfEmpty">After the country is absorbed, remove the source country from the countries array.</param>
        public bool CountryTransferCountry(int countryIndex, int sourceCountryIndex, bool redraw, bool mergeRegions = true, bool deleteSourceCountryIfEmpty = true) {
            ...
            return CountryTransferCountryRegion(countryIndex, sourceCountryRegion, redraw, mergeRegions, deleteSourceCountryIfEmpty);
        }
        ...
        /// <param name="mergeRegions">Merge resulting regions into a single region (if they're overlapping or intersect). If false, source country will be added as new region without any merging (faster).</param>
        /// <param name="deleteSourceCountryIfEmpty">After the country is absorbed, remove the source country from the countries array.</param>
        public bool CountryTransferCountryRegion(int countryIndex, Region sourceCountryRegion, bool redraw, bool mergeRegions = true, bool deleteSourceCountryIfEmpty = true) {
        ...
        /// </summary>
        /// <param name="deleteSourceCountryIfEmpty">If a source country is left without any province, remove it from the countries array</param>
        public bool CountryTransferProvinces(int targetCountryIndex, List<Province> provinces, bool redraw, bool deleteSourceCountryIfEmpty = true) {
        ...
        public void SetCountriesGeodata(string s) {
            int countryCount = s.Count('|') + 1;
            countries = new Country[countryCount];
            Vector2 min = Misc.Vector2one * 10;
            Vector2 max = -min;
            StringSegment[] countryInfo = new StringSegment[9];
            int k = 0;
            foreach (var countryData in s.GetSplit('|')) {
                int countryInfoLength = s.GetSplitSegments(countryData, '$', countryInfo);
                string name = s.GetSplitEntry(countryInfo[0]);
                string continent = s.GetSplitEntry(countryInfo[1]);
                int uniqueId;
                if (countryInfoLength >= 5) {
                    uniqueId = int.Parse(s.GetSplitEntry(countryInfo[4]));
                } else {
                ...
                foreach (var regionInfo in s.GetSplit('*', countryInfo[2])) {
                    int coorCount = s.Count(';', regionInfo) + 1;
                    if (coorCount < 3)
                        continue;
                    min.x = min.y = 10;
                    max.x = max.y = -10;
                    ...
                    int c = 0;
                    foreach (var coordinate in s.GetSplit(';', regionInfo)) {
                        GetPointFromPackedString(ref s, coordinate, out float x, out float y);
                        if (x < min.x)
                            min.x = x;
                        if (x > max.x)
                            max.x = x;
                ...
                if (countryInfoLength >= 4) {
                    if (int.TryParse(s.GetSplitEntry(countryInfo[3]), out int hidden)) {
                        country.hidden = hidden > 0;
                    }
                }
                // fip 10 4
                if (countryInfoLength >= 6) {
                    country.fips10_4 = s.GetSplitEntry(countryInfo[5]);
                }
                // iso A2
                if (countryInfoLength >= 7) {
                    country.iso_a2 = s.GetSplitEntry(countryInfo[6]);
                }
                // iso A3
                if (countryInfoLength >= 8) {
                    country.iso_a3 = s.GetSplitEntry(countryInfo[7]);
                }
                // iso N3
                if (countryInfoLength >= 9) {
                    country.iso_n3 = s.GetSplitEntry(countryInfo[8]);
                }
            ...
            for (int k = 0; k < countries.Length; k++) {
                ...
                for (int r = 0; r < regionsCount; r++) {
                    ...
                    for (int p = 0; p < pointsCount; p++) {
                        ...
                        int x = (int)(region.points[p].x * MAP_PRECISION);
                        int y = (int)(region.points[p].y * MAP_PRECISION);
        ...
        public byte[] GetCountriesGeodataBinary() {
            ...
            for (int k = 0; k < countriesCount; k++) {
                ...
                for (int r = 0; r < regionsCount; r++) {
                    ...
                    for (int p = 0; p < pointsCount; p++) {
                        bw.Write(region.points[p].x);
                        bw.Write(region.points[p].y);
                    }
                }
                bw.Write(country.hidden);
                bw.Write(country.uniqueId);
        ...
        public void SetCountriesGeodataBinary(byte[] data) {
            ...
            for (int k = 0; k < countryCount; k++) {
                ...
                Country country = new Country(name, continent, 0);
                country.regions = new List<Region>();
                float maxVol = 0;
                Vector2 minCountry = new Vector2(10, 10);
                Vector2 maxCountry = -minCountry;
                ...
                country.uniqueId = br.ReadInt32();
                if (country.uniqueId <= 0) {
                    country.uniqueId = GetUniqueId(new List<IExtendableAttribute>(_countries));
                }
