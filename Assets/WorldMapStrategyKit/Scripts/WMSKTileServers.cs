...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        static readonly string[] subservers = new string[] { "a", "b", "c" };
        ...
        public string GetTileURL(TILE_SERVER server, TileInfo ti, bool forceHTTPS) {
            ...
            if (!server.IsMapBox() && !server.IsAerisWeather() && server != TILE_SERVER.Custom && !string.IsNullOrEmpty(_tileServerAPIKey)) {
            ...
            if (forceHTTPS) {
                ...
                if (UnityEditor.PlayerSettings.insecureHttpOption != UnityEditor.InsecureHttpOption.AlwaysAllowed) {
                    Debug.LogWarning("Enable non-secure connections in Player Settings or try enabling 'Use Secure Connection' option in World Map Strategy Kit inspector.");
                }
            }
#endif
