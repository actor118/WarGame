        ...
        public Renderer ToggleCellSurface(int cellIndex, bool visible, Color color, bool refreshGeometry, Texture2D texture, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool autoChooseTransparentMaterial) {
            ...
            if (surf != null) {
                ...
                if (surfMaterial == null ||
                    ...
                    Material goodMaterial = GetColoredTexturedMaterial(color, texture, false, autoChooseTransparentMaterial);
                    if (_gridCutOutBorders && _gridAphaOnWater < 1f) {
                        goodMaterial.EnableKeyword(ShaderParams.SKW_WATER_MASK);
                    } else
                    {
                        goodMaterial.DisableKeyword(ShaderParams.SKW_WATER_MASK);
                    }
                    cell.customMaterial = goodMaterial;
                    ApplyMaterialToSurface(surf, goodMaterial);
                }
            } else {
                surfMaterial = GetColoredTexturedMaterial(color, texture, false, autoChooseTransparentMaterial, 2);
                if (_gridCutOutBorders && _gridAphaOnWater < 1f) {
                    surfMaterial.EnableKeyword(ShaderParams.SKW_WATER_MASK);
                } else
                {
                    surfMaterial.DisableKeyword(ShaderParams.SKW_WATER_MASK);
                }
