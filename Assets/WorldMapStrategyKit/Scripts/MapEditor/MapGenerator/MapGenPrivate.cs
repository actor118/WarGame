...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK_Editor : MonoBehaviour {
        ...
        void SaveGeneratedMapData() {
            ...
            // Save country data
            if (countryChanges) {
                fullPathName = path + "/" + GetCountryGeoDataBinaryFileName();
                byte[] data = _map.GetCountriesGeodataBinary();
            ...
            // Save province data
            if (provinceChanges) {
                fullPathName = path + "/" + GetProvinceGeoDataBinaryFileName();
                byte[] data = _map.GetProvincesGeodataBinary();
                File.WriteAllBytes(fullPathName, data);
                AssetDatabase.ImportAsset(fullPathName);
            ...
            // Save cities
            if (cityChanges) {
                fullPathName = path + "/" + GetCityGeoDataBinaryFileName();
                byte[] data = _map.GetCitiesGeodataBinary();
                File.WriteAllBytes(fullPathName, data);
                AssetDatabase.ImportAsset(fullPathName);
                cityChanges = false;
            }
