    ...
    public partial class WMSK_Editor : MonoBehaviour {
        ...
        void CountryHighlightSelection(List<Region> filterRegions) {
            ...
            if (country.regions != null && countryRegionIndex >= 0 && countryRegionIndex < country.regions.Count) {
                ...
                foreach (Region neighbourRegion in region.neighbours) {
            ...
            for (int k = 0; k < continents.Count; k++) {
                ...
                if (count > max) {
                    ...
                    for (int j = 0; j < count; j++) {
                        ...
                        if (neighbourCount > 0) {
                            foreach (Region reg in country.mainRegion.neighbours) {
                                otherCountry = (Country)reg.entity;
                                otherRegion = reg;
        ...
        public string GetCountryGeoDataBinaryFileName() {
            return map.frontiersDetail == FRONTIERS_DETAIL.Low ? "countries110_bin.txt" : "countries10_bin.txt";
        }
