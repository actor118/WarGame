...
namespace WorldMapStrategyKit {
    ...
    public static class Drawing {
        ...
        public static Renderer CreateSurface(string name, Vector3[] points, int[] indices, Material material, Rect rect, Vector2 textureScale, Vector2 textureOffset, float textureRotation, DisposalManager disposalManager, bool addVerticesPositionsAsUV2 = false) {
            ...
            if (uv == null || uv.Length < pointsCount) {
                uv = new Vector2[pointsCount];
            }
            ...
            for (int k = 0; k < pointsCount; k++) {
                ...
                Vector2 normCoor = new Vector2((coor.x - xMin) / w, (coor.y - yMin) / h);
                uv[k] = normCoor;
            }
            mesh.SetUVs(0, uv, 0, pointsCount);
            ...
            if (addVerticesPositionsAsUV2) {
                if (uv2 == null || uv2.Length < pointsCount) {
                    uv2 = new Vector2[pointsCount];
                }
                for (int k = 0; k < pointsCount; k++) {
        ...
        public static GameObject CreateSurface(string name, Polygon poly, Material material, Rect rect, Vector2 textureScale, Vector2 textureOffset, float textureRotation, DisposalManager disposalManager) {
            ...
            if (newPoints == null || newPoints.Length < triCount * 3) {
                newPoints = new Vector3[triCount * 3];
            }
            ...
            for (int k = 0; k < triCount; k++) {
                ...
                p.x = dt.Points[0].Xf;
                p.y = dt.Points[0].Yf;
                if (hit.TryGetValue(p, out int ptmp)) {
                    triNew[k * 3] = ptmp;
                } else {
                    newPoints[++newPointsCount] = p;
            ...
            int vertexCount = newPointsCount + 1;
            mesh.SetVertices(newPoints, 0, vertexCount);
            ...
            for (int k = 0; k < vertexCount; k++) {
                Vector3 np = newPoints[k];
                coor.x = np.x / textureScale.x;
                coor.y = np.y / textureScale.y;
                if (textureRotation != 0) {
                    coor = RotatePoint(coor, Misc.Vector2zero, textureRotation);
                }
                coor.x += textureOffset.x;
                coor.y += textureOffset.y;
                uv[k].x = (coor.x - xMin) / w;
                uv[k].y = (coor.y - yMin) / h;
            }
