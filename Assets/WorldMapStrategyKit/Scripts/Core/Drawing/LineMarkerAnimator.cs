...
namespace WorldMapStrategyKit {
    ...
    public class LineMarkerAnimator : MonoBehaviour {
        ...
        void DoInit() {
            ...
            useArrowStartCap = usesViewport && startCap != null;
            useArrowEndCap = usesViewport && endCap != null;
            isFading = false;
        ...
        void CreateDashedLineVertices() {
            ...
            Vector2 mapPos;
            Vector3 worldPos = Misc.Vector3zero;
            Vector3 prevWorldPos = Misc.Vector3zero;
            for (float distanceAcum = startingDistance; distanceAcum < totalDistance + step; distanceAcum += dashInterval, pair++) {
                ...
                if (usesViewport) {
                    ...
                    worldPos = map.Map2DToWorldPosition(mapPos, elevation, HEIGHT_OFFSET_MODE.ABSOLUTE_CLAMPED, false);
                    if (vertices.Count > 0 || (pair % 2 == 0) && prevWorldPos != worldPos) {
        ...
        void UpdateDashedLine(float t) {
            ...
            if (currentVertex >= 0 && currentVertex < vertexCount) {
                ...
                if (useArrowEndCap && t > 0.1f) {
                    ...
                    ProjectIntoWorldSpace(ref mapPos, t, t0);

                    if (drawingDuration == 0) {
                        // forced code to get the previous position for the cap base
                        t = 0.99f;
                        index = (int)((path.Length - 1) * t);
                        findex = Mathf.Min(index + 1, path.Length - 1);
                        t0 = t * (path.Length - 1);
                        t0 -= index;
                        Vector3 prevMapPos = Vector2.Lerp(path[index], path[findex], t0);
                        ProjectIntoWorldSpace(ref prevMapPos, t, t0);
                        endCapArrowBasePos = prevMapPos;
                    } else {
                        endCapArrowBasePos = endCapArrowTipPos;
                    }
            ...
            void ProjectIntoWorldSpace(ref Vector3 mapPos, float t, float t0) {
                if (usesViewport) {
