...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        void SetupViewport () {
            ...
            switch (viewportMode) {
                ...
                case ViewportMode.Terrain:
                    ...
                    aspect = terrainSize.x / terrainSize.z;
                    break;
                default: {
        ...
        void UpdateWrapCam () {
            ...
            Transform currentCameraTransform = _currentCamera.transform;
            Transform wrapCameraObjTransform = _wrapCameraObj.transform;

            Vector3 camToMapVector = currentCameraTransform.forward * lastDistanceFromCamera;

            // Reduce floating-point errors
            Vector3 apos = transform.position;
            transform.position -= apos;
            currentCameraTransform.position -= apos;

            // Get clip bounds
            Vector3 v0 = _currentCamera.WorldToViewportPoint(transform.TransformPoint(new Vector3(-0.5f, 0, 0)));
            Vector3 v1 = _currentCamera.WorldToViewportPoint(transform.TransformPoint(new Vector3(0.5f, 0, 0)));
            ...
            if (x0 > 1) {
                ...
                currentCameraTransform.position = _currentCamera.ViewportToWorldPoint(v);
                currentCameraTransform.position -= camToMapVector;
            ...
            if (x0 > 0) {
                ...
                wrapCameraObjTransform.position = _currentCamera.ViewportToWorldPoint(v);
            }
            else if (x1 < 1) {
                // wrap on the right
            ...
            wrapCameraObjTransform.rotation = currentCameraTransform.rotation;
            wrapCameraObjTransform.position -= camToMapVector;

            // Restore positions
            transform.position += apos;
            currentCameraTransform.position += apos;
            wrapCameraObjTransform.position += apos;
        ...
        void ComputeViewportRect (bool useSceneViewWindow = false) {
            ...
            renderViewportClip0 = cam.WorldToViewportPoint(topLeft);
            Vector3 bottomRight = transform.TransformPoint(0.5f, -0.5f, 0);
            renderViewportClip1 = cam.WorldToViewportPoint(bottomRight);
