...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        Vector2 ConvertToTextureCoordinates(Vector3 vertex, Rect rect, int width, int height) {
            Vector2 v;
            v.x = (int)((vertex.x - rect.xMin) / rect.width * width);
            v.y = (int)((vertex.y - rect.yMin) / rect.height * height);
            return v;
        }
        ...
        public void RegionPaint(Color[] colors, int textureWidth, int textureHeight, Region region, Color color, bool alphaBlending, bool useWorldRelativeSize = true) {
            ...
            for (int i = 0; i < indices.Length; i += 3) {
                ...
                if (useWorldRelativeSize) {
                    p1 = ConvertToTextureCoordinates(vertices[indices[i]], textureWidth, textureHeight);
                    p2 = ConvertToTextureCoordinates(vertices[indices[i + 1]], textureWidth, textureHeight);
                    p3 = ConvertToTextureCoordinates(vertices[indices[i + 2]], textureWidth, textureHeight);
                } else {
                    p1 = ConvertToTextureCoordinates(vertices[indices[i]], region.rect2D, textureWidth, textureHeight);
                    p2 = ConvertToTextureCoordinates(vertices[indices[i + 1]], region.rect2D, textureWidth, textureHeight);
                    p3 = ConvertToTextureCoordinates(vertices[indices[i + 2]], region.rect2D, textureWidth, textureHeight);
                }
        ...
        GameObject DrawRegionOutlineMesh(string name, Region region, bool overridesAnimationSpeed = false, float animationSpeed = 0f, bool reuseMaterial = true) {
            ...
            if (_outlineDetail == OUTLINE_DETAIL.Simple && !customBorder) {
                ...
                lr.positionCount = pointsLength + 1;
                lr.textureMode = LineTextureMode.Tile;
                for (int k = 0; k < pointsLength; k++) {
                    lr.SetPosition(k, region.points[k]);
                }
                lr.SetPosition(pointsLength, region.points[0]);
                lr.loop = true;
        ...
        Material GetRegionCustomBorderMaterial(Region region, float animationSpeed) {
            ...
            if (!customBorderMaterialsDict.TryGetValue(customBorder, out Material mat)) {
