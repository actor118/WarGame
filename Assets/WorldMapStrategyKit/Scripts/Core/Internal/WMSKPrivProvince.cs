...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        public void ReadProvincePackedString(IAdminEntity entity) {
            ...
            Vector2 minProvince = Misc.Vector2one * 10;
            Vector2 maxProvince = -minProvince;
            foreach (var region in province.packedRegions.GetSplit('*')) {
                int coorCount = province.packedRegions.Count(';', region) + 1;
                ...
                int c = 0;
                foreach (var coordinate in province.packedRegions.GetSplit(';', region)) {
                    GetPointFromPackedString(ref province.packedRegions, coordinate, out float x, out float y);
                    if (x < min.x)
                        min.x = x;
                    if (x > max.x)
                        max.x = x;
                ...
                if (vol > maxVol) {
                    maxVol = vol;
                    province.mainRegionIndex = province.regions.Count - 1;
                    province.center = provinceRegion.center;
                }
            }
            province.regionsRect2D = new Rect(minProvince.x, minProvince.y, Math.Abs(maxProvince.x - minProvince.x), Mathf.Abs(maxProvince.y - minProvince.y));
        ...
        bool mDrawProvinces(int countryIndex, bool includeNeighbours, bool forceRefresh, bool justComputeBorders) {
            ...
            if (_drawAllProvinces) {
                ...
                if (includeNeighbours) {
                    foreach (Region region in _countries[countryIndex].regions) {
                        foreach (Region neighbour in region.neighbours) {
                            Country c = (Country)neighbour.entity;
