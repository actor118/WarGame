    ...
    public partial class WMSK : MonoBehaviour {
        ...
        void ReloadFont() {
            if (_countryLabelsTextEngine == TEXT_ENGINE.TextMeshStandard) {
                ...
                if (disposalManager != null) {
                    disposalManager.MarkForDisposal(fontMaterial);
                }
                labelsFont.material = fontMaterial;
                labelsShadowMaterial = Instantiate(fontMaterial);
                if (disposalManager != null) {
                    disposalManager.MarkForDisposal(labelsShadowMaterial);
                }
...
#else
                labelsFontTMPro = (TMP_FontAsset)Instantiate(_countryLabelsFontTMPro);
                labelsFontTMProMaterial = _countryLabelsFontTMProMaterial;
                if (labelsFontTMPro != null) {
                    if (labelsFontTMProMaterial == null) {
                        labelsFontTMProMaterial = Instantiate(labelsFontTMPro.material);
                        disposalManager?.MarkForDisposal(labelsFontTMProMaterial);
                    }
                    labelsFontTMPro.material = labelsFontTMProMaterial;
                }
        ...
        void DrawUnityStandardTextLabels() {
            ...
            for (int countryIndex = 0; countryIndex < _countries.Length; countryIndex++) {
                ...
                if (_countryLabelsAlwaysOnTop && _countryLabelsRenderingMode == TEXT_RENDERING_MODE.FloatingAboveViewport) {
                    tm.font.material.SetInt(ShaderParams.ZTest, (int)UnityEngine.Rendering.CompareFunction.Always);
                } else {
                    tm.font.material.SetInt(ShaderParams.ZTest, (int)UnityEngine.Rendering.CompareFunction.LessEqual);
                }
        ...
        void UpdateFloatingLabel(AdminEntity entity) {
            ...
            if (_countryLabelsUseMapCurvature && currentCurvature != 0) {
                float ang = labelPos.x * Mathf.PI * currentCurvature * 0.01f;
                t.Rotate(renderViewport.transform.up, ang * Mathf.Rad2Deg, Space.World);
            }
