...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        bool CheckRouteLandAndWaterMask() {
            ...
            maskLoadedFlag = 0;
            ConvertMaskToBitArray(pathFindingWaterMap, 1);
            ConvertMaskToBitArray(pathFindingLandMap, 2);

            while(maskLoadedFlag != 3) {
                Thread.Sleep(10);
            }
        ...
        void ConvertMaskToBitArray(Texture2D tex, int loadFlag) {
            if (tex == null) return;
            if (tex.format == TextureFormat.R8) {
                ...
                if (expectedLength == rawData.Length) {
                    if (Application.isPlaying) {
                        Task.Run(() => {
                            if (loadFlag == 2) {
                                pathfindingLandMask = new FastBitArray(rawData, 128, 255);
                            } else {
                                pathfindingWaterMask = new FastBitArray(rawData, 128, 255);
                            }
                            maskLoadedFlag |= loadFlag;
                        });
                    } else {
                        if (loadFlag == 2) {
                            pathfindingLandMask = new FastBitArray(rawData, 128, 255);
            ...
            if (Application.isPlaying) {
                Task.Run(() => {
                    if (loadFlag == 2) {
                        pathfindingLandMask = new FastBitArray(colors, 128, 255);
                    } else {
                        pathfindingWaterMask = new FastBitArray(colors, 128, 255);
                    }
                    maskLoadedFlag |= loadFlag;
                });
            } else {
