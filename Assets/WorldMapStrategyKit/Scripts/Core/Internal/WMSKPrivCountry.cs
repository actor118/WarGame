...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        void TestNeighbourSegmentAny (Region region, int i, int j) {
            ...
            if (frontiersCacheHit.TryGetValue(v, out Region neighbour)) { // add neighbour references
                if (neighbour != region && region.neighbours.Add(neighbour)) {
        ...
        void TestNeighbourSegmentInland (Region region, int i, int j) {
            ...
            if (frontiersCacheHit.TryGetValue(v, out neighbour)) { // add neighbour references
                if (neighbour != region) {
                    if (region.neighbours.Add(neighbour)) {
                        neighbour.neighbours.Add(region);
                    }
                    frontiersPoints.Add(region.points[i]);
                    frontiersPoints.Add(region.points[j]);
        ...
        void DrawFrontiers () {
            ...
            if (needOptimizeFrontiers || frontiers == null) {
        ...
        void CountrySubstractProvinceEnclaves (int countryIndex, Region region, Polygon poly) {
        ...
        /// <summary>
        /// Creates a colored or texture surface of a country region
        /// </summary>
        GameObject internal_ToggleCountryRegionSurface (int countryIndex, int regionIndex, bool visible, Color color, Texture2D texture = null, Vector2 textureScale = default, Vector2 textureOffset = default, float textureRotation = 0) {
            ...
            return internal_ToggleCountryRegionSurface(countryIndex, regionIndex, visible, surfOptions);
        }

        /// <summary>
        /// Creates a colored or texture surface of a country region
        /// </summary>
        GameObject internal_ToggleCountryRegionSurface (int countryIndex, int regionIndex, bool visible, SurfaceOptions options) {
            ...
            if (surf != null) {
                ...
                Material goodMaterial = GetColoredTexturedMaterial(options.color, options.texture, options.useSDFGradient, true);
                ...
                surfMaterial = GetColoredTexturedMaterial(options.color, options.texture, options.useSDFGradient, true);
                surf = GenerateCountryRegionSurface(countryIndex, regionIndex, surfMaterial, options.textureScale, options.textureOffset, options.textureRotation);
        ...
        /// <summary>
        /// Internal method to transfer provinces between countries
        /// </summary>
        /// <param name="deleteSourceCountryIfEmpty">If source country has no provinces left, remove it from the countries array.</param>
        void internal_CountryTransferProvinces (int targetCountryIndex, List<Province> provinces, int sourceCountryIndex, bool deleteSourceCountryIfEmpty = true) {
