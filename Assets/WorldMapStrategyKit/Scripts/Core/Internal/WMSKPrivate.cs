...
namespace WorldMapStrategyKit {
    ...
    public partial class WMSK : MonoBehaviour {
        ...
        public static float mapWidth { get { return instanceExists ? instance.transform.localScale.x : 200.0f; } }

        public static float mapHeight { get { return instanceExists ? instance.transform.localScale.y : 100.0f; } }
        ...
        static void DomainInit () {
            WMSK[] maps = Misc.FindObjectsOfType<WMSK>(true);
        ...
        void OnDisable () {
            ...
            if (!Application.isPlaying && disposalManager != null) {
        ...
        void Update () {
            ...
            // Check boundaries
            Vector3 currentCameraPosition = _currentCamera.transform.position;
            if (transform.position != lastMapPosition || currentCameraPosition != lastCamPosition || _currentCamera.orthographicSize != lastCamOrtographicSize) {
                ...
                if (_currentCamera.orthographic) {
                    ...
                    if (!_enableFreeCamera && (_allowUserZoom || flyToActive) && (_zoomMinDistance > 0 || _zoomMaxDistance > 0)) {
                        ...
                        if (lastDistanceFromCamera < minDistance) {
                            _currentCamera.transform.position = ClampDistanceToMap(prevCamPos, currentCameraPosition, lastDistanceFromCamera, minDistance);
                            ...
                            if (lastDistanceFromCamera > maxDistance) {
                                ...
                                _currentCamera.transform.position = ClampDistanceToMap(prevCamPos, currentCameraPosition, lastDistanceFromCamera, maxDistance);
        ...
        void CheckRectConstraints () {
            ...
            if (clamp) {
                pos = cam.ViewportToWorldPoint(pos);
                Vector3 offset = posEdge - pos;
                offset = transform.InverseTransformDirection(offset);
                offset.x = 0; // Do not affect the x-axis
                offset.z = 0; // Do not affect the z-axis
        ...
        void PerformUserInteraction () {
            ...
            if (pinching && ((wheelAccel == 0 && !input.touchSupported) || (input.touchSupported && input.touchCount == 0))) {
                pinching = false;
            }
            ...
            if (!pinching) {
                if (mouseIsOver) {
                    ...
                    bool goodHit = GetLocalHitFromMousePos(out Vector3 localPoint);
                    ...
                    if (buttonIndex >= 0 && canInteractWhileFlying) {
                        if (releasing) {
                            ...
                            if (elapsedTimeSincePress < TAP_THRESHOLD && dragDampingStart < 0.001f) {
        ...
        public void Init () {
            ...
            if (countries == null) {
                // countries == null is often used to determine if initialization is done. So we set it to an array of zero instead here to enable working with the assets with no countries at all.
        ...
        readonly SurfaceOptions surfOptions = new SurfaceOptions();
        ...
        float GetFrustumDistance (Camera cam) {
            ...
            if (cam.orthographic) {
                ...
                float tan = 0.5f / Mathf.Tan(radAngle * 0.5f);
                frustumDistanceH = mapHeight * _windowRect.height * tan;
                frustumDistanceW = mapWidth * (_windowRect.width / aspect) * tan;
                if (_fitWindowWidth && _fitWindowHeight) {
        ...
        Material GetColoredTexturedMaterial (Color color, Texture2D texture) {
            return GetColoredTexturedMaterial(color, texture, false, true);
        }


        Material GetColoredTexturedMaterial (Color color, Texture2D texture, bool ignoreMaterialCache, bool autoChooseTransparentMaterial, int renderQueueIncrement = 0) {
            ...
            if (!cacheMaterials) {
                ignoreMaterialCache = true;
            }

            if (!ignoreMaterialCache && coloredMatCache.TryGetValue(hash, out customMat)) {
                customMat.renderQueue += renderQueueIncrement;
                return customMat;
            }

            if (ignoreMaterialCache || texture != null) {
        ...
        void GetPointFromPackedString (ref string s, StringSegment segment, out float x, out float y) {
            int d = 1;
            float v = 0;
            y = 0;
            int lastIndex = segment.startIndex + segment.length;
            for (int k = lastIndex - 1; k >= segment.startIndex; k--) {
                char ch = s[k];
                if (ch >= '0' && ch <= '9') {
                    v += (ch - '0') * d;
                    d *= 10;
