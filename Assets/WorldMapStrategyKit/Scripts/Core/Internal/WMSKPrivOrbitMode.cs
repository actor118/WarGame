        ...
        void ApplyCameraTilt() {
            ...
            if (_enableCameraOrbit) {
                if (input.GetKey(_orbitLeftKeyName)) {
                    _orbitYawAngle += _orbitRotationSpeed;
                    _orbitYawAngle = (_orbitYawAngle + 360) % 360f;
                ...
                float destYawAngle;
                if (_orbitYawAngle - currentYawAngle > 180) {
                    destYawAngle = _orbitYawAngle - 360f;
                } else if (currentYawAngle - _orbitYawAngle > 180) {
                    destYawAngle = _orbitYawAngle + 360f;
                } else {
                    destYawAngle = _orbitYawAngle;
                }
