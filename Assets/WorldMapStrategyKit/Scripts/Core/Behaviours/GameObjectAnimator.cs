...
namespace WorldMapStrategyKit {
    ...
    public partial class GameObjectAnimator : MonoBehaviour, IExtendableAttribute {
        ...
        [Tooltip("The total distance in km from start and destination when a move command is issued")]
        public float totalDistance;

        [Tooltip("The % of total distance travelled since movement started")]
        ...
        /// - ABSOLUTE_CLAMPED: gameobject is set to given altitude but never below terrain.
        ...
        /// <param name="altitude">Destination altitude.</param>
        public bool MoveTo (Vector2 destination, float duration, DURATION_TYPE durationType = DURATION_TYPE.Step, float altitude = -1) {
            altitudeStart = currentAltitude;
            altitudeEnd = altitude >= 0 ? altitude : currentAltitude;
            if (terrainCapability != TERRAIN_CAPABILITY.Any) {
                ...
                totalDistance = Conversion.Distance(_currentMap2DLocation, destination) / 1000f;

                // Duration?
                if (durationType == DURATION_TYPE.MapLap) {
                    float distance = Vector2.Distance(_currentMap2DLocation, destination);
                    duration *= distance;
                }
        ...
        public bool MoveTo (List<Vector2> route, float duration, DURATION_TYPE durationType = DURATION_TYPE.Step) {
            ...
            for (int k = 1; k < routeCount; k++) {
                totalDistance += Conversion.Distance(prevPos, route[k]);
                prevPos = route[k];
            }
            totalDistance /= 1000f;
        ...
        /// <summary>
        /// Immediately repositions a unit to a specific destination
        /// </summary>
        public void TeleportTo (Vector2 destination) {
            _currentMap2DLocation = startingMap2DLocation = endingMap2DLocation = destination;
            isMoving = false;
        ...
        void MoveGameObject () {
            ...
            if (progress >= 1f) { // end of movement - clear holdPosition flag (used for lookat)
                holdPosition = false;
                ...
                if (route != null) {
                    ...
                    if (canMove) {
                        ...
                        hasMoved = true;
                        distanceTravelled = totalDistance * progress;
                    }
                }
                else {
                    if (canMove) {
        ...
        public void UpdateVisibilityWithContext (bool forceRefreshVisibility) {
            ...
            if (shouldBeVisible != isVisibleInViewport || forceRefreshVisibility) {
                ...
                if (map.OnVGOVisibleChange != null) {
                    map.OnVGOVisibleChange(this);
                }
