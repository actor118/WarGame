...
*/
					...
					half4 water = getWater(i.uv, terra);
					half4 earth;
						// compute Earth pixel color
						if (i.uv.x<0.5) {
							if (i.uv.y>0.5) {
								earth = tex2Dlod(_TexTL, float4(i.uv.x * 2.0, (i.uv.y - 0.5) * 2.0, 0, 0));
							} else {
								earth = tex2Dlod(_TexBL, float4(i.uv.x * 2.0, i.uv.y * 2.0, 0, 0));
							}
						} else {
							if (i.uv.y>0.5) {
								earth = tex2Dlod(_TexTR, float4((i.uv.x - 0.5) * 2.0f, (i.uv.y - 0.5) * 2.0, 0, 0));
							} else {
								earth = tex2Dlod(_TexBR, float4((i.uv.x - 0.5) * 2.0f, i.uv.y * 2.0, 0, 0));
							}
						}
						earth.a = 0;
						...
						half luma = earth.g;
					#else
						half luma = LinearToGammaSpace(earth.rgb).g;
					#endif
					elevation -= luma < 0.2;
					if (_WaterLevel.x) {
