Shader "WMSK/Unlit Earth Scenic 16K" {
...
Pass {
			...
			v2f vert(AppData v) {
             ...
             half4 frag (v2f i) : SV_Target {
					...
					if (i.uv.x<0.5) {
						if (i.uv.y>0.5) {
							earth = tex2Dlod(_TexTL, float4(i.uv.x * 2.0, (i.uv.y - 0.5) * 2.0, 0, mip));
						} else {
							earth = tex2Dlod(_TexBL, float4(i.uv.x * 2.0, i.uv.y * 2.0, 0, mip));
						}
					} else {
						if (i.uv.y>0.5) {
							earth = tex2Dlod(_TexTR, float4((i.uv.x - 0.5) * 2.0f, (i.uv.y - 0.5) * 2.0, 0, mip));
						} else {
							earth = tex2Dlod(_TexBR, float4((i.uv.x - 0.5) * 2.0f, i.uv.y * 2.0, 0, mip));
						}
					}
