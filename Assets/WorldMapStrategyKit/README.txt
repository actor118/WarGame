...
Version 16.0
- Internal changes to improve performance. Region.neighbours is now a HashSet.
- Improved performance of SanitizeRegions method
- Map Editor: minor city inspector improvements
- [Fix] Added missing capitals to certain minor countries

Version 15.9.2
- [Fix] Fixed shaking issue when dragging the map along the North/South edges at very low speed
- [Fix] Marker material now exposes a z-offset property to customize the order of appearance

Version 15.9
- API: Changed AddCircle / AddCircleOnSphere overdraw & renderQueue parameters to be more intuitive
- Fixes and minor improvements

Version 15.8
- API: added WMSK_Teleport extension method to gameobjects
- API: added Teleport method to GameObjectAnimator component

Version 15.7
- API: added OnVGOVisibleChange event. Triggered when any unit visibilty changes in the viewport.
- Added "Use Secure Connection" option to Tile System section

Version 15.6
- New demo scene 008: SDF Region
- API: new ToggleCountryMainRegionSurface / ToggleCountryRegionSurface with support for SDF borders
- API: new ToggleProvinceRegionSurface / ToggleProvinceRegionSurface with support for SDF borders
- [Fix] Map Editor: fixes to save/restore backups in binary format

Version 15.5
- Grid mode now works without countries/provinces or any other geodata information
- API: CountryTransferCountry(): added deleteSourceCountry parameter
- API: added AddMarker2DTextPro which uses TextMeshPro
- [Fix] Fixed country frontiers thicker geometry shader compilation

Version 15.4
- Added "Scenic 16K" Earth style
- Removed dependency of standard font when using Text Mesh Pro
- [Fix] VR fixes

Version 15.3
- Geodata now provided in binary format for improved loading times and reduced disk usage
- Added "Geodata Format" to inspector (can be used to revert back to packed string format when loading or saving changes with Map Editor)
- [Fix] API: fixed point order issue with GetProvinceBorderPoints()

Version 15.2
- "Respect Other UI" now also blocks events on markers
- [Fix] Fixes for Map Transform tool in Map Editor
- [Fix] Fixes for Unity 2023.3

Version 15.1
- Online tile system: ability to store tiles in file system and streaming assets folder
- Country floating labels: added option to render "Always On Top"
- Country floating labels: added option to render labels curved to respect map curvature
- Exposed Yaw angle property under orbit section
- Units moving using FingPath method now ignore the starting terrain type to avoid situations where unit couldn't exit a starting port or base
- API: added [Get/Set[]Countries/Provinces/Cities]GeodataBinary methods to load/save data in pure binary format

Version 15.0
- Minimum Unity version required is now Unity 2021.3.16
- [Fix] Fixes & improvements to the line renderering system

Version 14.8
- [Fix] Fixes & improvements to the line renderering system

Version 14.7.2
- Added utility buttons to WMSK inspector at top
- [Fix] Fixed non-animated lines being invisible bug

Version 14.7
- Change: frontiers are now not visible by default when creating a new WMSK instance by code
- Perf/memory optimization when loading land/water map textures
- Perf/memory optimization when loading geodata
- API: GameObjectAnimator: added totalDistance and distanceTravelled properties. These values are calculated and updated in realtime automatically when an unit moves.
